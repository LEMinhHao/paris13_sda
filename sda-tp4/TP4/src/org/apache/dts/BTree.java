 ...
 * Class BTree
 * 
 * @author tnguyen Description: BTree implementation
 */
public class BTree<K extends Comparable, V> {
	...
	public V search(K key) {
		...
		while (currentNode != null) {
			...
			// We don't need it
			/*
			 * if (currentNode.mIsLeaf) { return null; }
			 */
	...
	private void splitNode(BTNode parentNode, int nodeIdx, BTNode btNode) {
		...
		// The node at this point should have LOWER_BOUND_KEYNUM (aka min degree - 1)
		// keys at this point.
		...
		// Do the right shift all the keys of the parent node the right side of the node
		// index as well
	...
	private void performLeftRotation(BTNode<K, V> btNode, int nodeIdx, BTNode<K, V> parentNode,
		...
		// Move the leftmost key of the right sibling and relevant child pointer to the
		// parent node
	...
	private boolean performMergeWithLeftSibling(BTNode<K, V> btNode, int nodeIdx, BTNode<K, V> parentNode,
		...
		// Here we need to determine the parent node's index based on child node's index
		// (nodeIdx)
		if (nodeIdx > 0) {
	...
	//
	// Remove the specified key and move a key from the right leaf sibling to the
	// node
	...
	//
	// Remove the specified key and move a key from the left leaf sibling to the
	// node
	// Note: The node and its sibling must be leaves
	//
	...
	// Return false if we reach and update the root hence we don't need to go futher
	// for re-balancing the tree
	//
	...
	/**
	 * Inner class StackInfo for tracing-back purpose Structure contains parent node
	 * and node index
	 */
